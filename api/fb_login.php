...
if(!($fbid)){
	...
	if($email_exists){
	$sth=$conn->prepare("update users set fbid=:fbid where email=:email");
	$sth->bindValue("email",$email);	
	$sth->bindValue("fbid",$fbid);
	try{$sth->execute();}
	catch(Exception $e){}	
	
	$data= Users::fbsignin($fbid,$email);
	...
	values(DEFAULT,:apn_id,:fbid,'','','','',:name,:email,'',:bio,:profile_pic,:token,0,NOW())";
