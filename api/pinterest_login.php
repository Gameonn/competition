...
// + STEP 2: get data				   +
// +-----------------------------------+
$pinterest_id=$_REQUEST['pinterest_id'];
...
if(!($pinterest_id)){
	...
	if($email_exists){
	$sth=$conn->prepare("update users set pinterest_id=:pinterest_id where email=:email");
	$sth->bindValue("email",$email);	
	$sth->bindValue("pinterest_id",$pinterest_id);
	try{$sth->execute();}
	catch(Exception $e){}	
	
	$data= Users::pinsignin($pinterest_id);
	...
	$twid_exists=Users::checkpinterest($pinterest_id);
	...
	values(DEFAULT,:apn_id,'','','','',:pinterest_id,:name,:email,'',:bio,:profile_pic,:token,0,NOW())";
