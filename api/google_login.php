...
if(!($google_id)){
	...
	if($email_exists){
	$sth=$conn->prepare("update users set google_id=:google_id where email=:email");
	$sth->bindValue("email",$email);	
	$sth->bindValue("google_id",$google_id);
	try{$sth->execute();}
	catch(Exception $e){}	
	
	$data= Users::googlesignin($google_id);
	...
	$googleid_exists=Users::checkgoogle($google_id);
	...
	values(DEFAULT,:apn_id,'','','',:google_id,'',:name,:email,'',:bio,:profile_pic,:token,0,NOW())";
