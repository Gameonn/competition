...
if(!($instagram_id)){
	...
	if($email_exists){
	$sth=$conn->prepare("update users set instagram_id=:instagram_id where email=:email");
	$sth->bindValue("email",$email);	
	$sth->bindValue("instagram_id",$instagram_id);
	try{$sth->execute();}
	catch(Exception $e){}	
	
	$data= Users::instagramsignin($instagram_id);
	...
	$instid_exists=Users::checkinstagram($instagram_id);
	...
	values(DEFAULT,:apn_id,'','',:instagram_id,'','',:name,:email,'',:bio,:profile_pic,:token,0,NOW())";
